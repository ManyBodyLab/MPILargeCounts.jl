...
function mpi_ibarrier(
        blocking::Bool = true, comm::MPI.Comm = MPI.COMM_WORLD,
        req::MPI.AbstractRequest = MPI.Request()
    )
    (blocking && MPI.Initialized()) && (MPI.Ibarrier(comm, req))
    return nothing
end
...
Execute the function `F` with arguments `args...` and keyword arguments `kwargs...` on the root rank
...
Returns the result of `F` on the root rank, and `nothing` on all other ranks.
"""
...
Returns the result of `F` on all ranks.
"""
